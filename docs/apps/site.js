...
function paint(){pending=false;document.querySelectorAll('[data-parallax]').forEach(el=>{if(motion.matches||small.matches){el.style.transform='';return}const scene=el.closest('section')||el.closest('main');if(!scene)return;const rect=scene.getBoundingClientRect();if(rect.bottom>0&&rect.top<innerHeight){const y=scene.id==='profile'?-rect.top:innerHeight/2-(rect.top+rect.height/2);el.style.transform=`translate3d(0,${Math.max(-160,Math.min(160,y*Number(el.dataset.parallax)))}px,0)`}})}
...
if(capabilitySection)new MutationObserver(()=>{if(capabilitySection.classList.contains('is-minimized')&&activeCapability)closeCapability(false)}).observe(capabilitySection,{attributes:true,attributeFilter:['class']});
...
if(collapsibleSections.length)roadmapMain.append(roadmap);
...
// Default homepage stays collapsed; explicit section links reveal their destination.
