(()=>{
 ...
 let manual=false,blinkTimer,closing;
 ...
 function sync(){[blinkTimer,closing].forEach(clearTimeout);widget.classList.remove('is-blinking');widget.classList.toggle('is-paused',stopped());pause.setAttribute('aria-pressed',String(manual||media.matches));pause.disabled=media.matches;pause.textContent=media.matches?'Still':manual?'Play':'Pause';pause.setAttribute('aria-label',media.matches?'Motion reduced by device settings':manual?'Play Scout animation':'Pause Scout animation');if(!stopped()){scheduleBlink()}}
