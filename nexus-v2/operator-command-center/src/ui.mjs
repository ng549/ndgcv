...
function commandButton(worker, action) {
  ...
  if (action === "REASSIGN_MODEL") {
    input = '<input class="cmd-input" type="text" placeholder="model id" data-input="model" data-worker="' + escapeHtml(worker.id) + '"' + (allowed ? "" : " disabled") + ">";
  ...
  const caption = !allowed && reason ? '<div class="reason">' + escapeHtml(reason) + "</div>" : "";
...
const CLIENT_SCRIPT_LINES = [
  ...
  "  if(action==='REASSIGN_MODEL'){var inp=document.querySelector('input[data-input=\"model\"][data-worker=\"'+id+'\"]');if(!inp||!inp.value){toast('Model id required');return}body.model=inp.value;}",
  "  if(action==='ADJUST_BUDGET'){var inp2=document.querySelector('input[data-input=\"budget\"][data-worker=\"'+id+'\"]');var dollars=inp2?parseFloat(inp2.value):NaN;if(!isFinite(dollars)||dollars<0){toast('Budget in dollars required');return}body.budgetLimitMicros=Math.round(dollars*1000000);}",
  "  post('/operator/api/workers/'+encodeURIComponent(id)+'/command',body).then(function(){toast(action+' request accepted');setTimeout(function(){location.reload()},600)}).catch(function(e){toast(e.message)});",
  " };});",
  "}",
  "function bindGlobal(){",
...
<div class="global">
<button id="runAll"${gAll.allowed === true ? "" : " disabled"}${gAll.allowed !== true && gAll.reason ? ' title="' + escapeHtml(gAll.reason) + '"' : ""}>RUN ALL READY</button>
<input id="phase" placeholder="Phase"><button id="runPhase"${gPhase.allowed === true ? "" : " disabled"}${gPhase.allowed !== true && gPhase.reason ? ' title="' + escapeHtml(gPhase.reason) + '"' : ""}>RUN PHASE</button>
</div>
