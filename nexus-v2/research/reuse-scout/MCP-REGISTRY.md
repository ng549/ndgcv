...
| Shopify Storefront MCP / Dev MCP | Shopify | Storefront commerce actions and developer tooling | Official Shopify Storefront MCP is documented for real-time commerce data/cart operations; Shopify Dev MCP is official for app/platform development | Shopify store/app auth depending surface | INTEGRATE BY USE CASE | VERIFIED official; distinguish shopper Storefront MCP from developer MCP; Admin mutation scope still needs bounded tests |
| QuickBooks MCP / connectors | Intuit | Accounting and financial operations | Intuit publicly confirms MCP connectors powering QuickBooks integrations in Perplexity as of Aug 31 2026, but a generic public Intuit remote MCP endpoint for arbitrary Nexus clients was not verified | Product/account auth within supported partner integration | HOLD FOR DIRECT NEXUS MCP | VERIFIED that Intuit uses MCP connectors; UNVERIFIED generic public endpoint. Use direct QuickBooks API/Nango until Intuit exposes supported Nexus-compatible endpoint |
| Xero MCP Server | Xero | Accounting data/actions for agentic integrations | Xero officially publishes an open-source local/STDIO MCP server and AI Toolkit; Xero reports >1M API calls through the MCP by June 2026. Remote MCP is in beta testing | OAuth 2.0 / Xero app permissions; remote beta terms where applicable | INTEGRATE LOCAL/STDIO AFTER SCOPE TEST; REMOTE HOLD | VERIFIED official toolkit; remote MCP not GA. Developer pricing changed Mar 2 2026 and API data may not be used to train AI/ML models under revised terms |
| Salesforce Headless 360 MCP | Salesforce | CRM/platform data, records, users, Apex and event capabilities | Official Salesforce-hosted Headless 360 MCP is open beta; Data 360 MCP and several related surfaces are GA as of Aug 25 2026 | Salesforce org auth/permissions | CONDITIONAL | VERIFIED official. Headless 360 remains beta and includes highly privileged tools; use only narrowed scopes/non-production until GA/security review |
| Pipedrive Native MCP | Pipedrive | CRM deals, contacts, leads, activities, pipeline actions | Native MCP launched June 30 2026; official docs updated Sep 2026; Pipedrive reports full audit trail and inherited user permissions | OAuth; inherits Pipedrive role/visibility | INTEGRATE | VERIFIED official current server; writes still require Nexus approval policy for sensitive records |
