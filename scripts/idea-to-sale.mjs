...
export function ideaToSale(e) {
  return `<section class="brand-story idea-sale-section" id="product-journey" aria-labelledby="idea-sale-heading"><div class="shell"><div class="section-heading" id="connecting-ideas"><span class="eyebrow">Connecting the idea with the operation</span><h2 id="idea-sale-heading">From idea to sale</h2><p>I bring the commercial decisions and the practical work together, from shaping an offer to getting it into customers’ hands. These parts overlap: I work through them with the people responsible, returning to earlier decisions as we learn more.</p></div><ol class="idea-stages">${stages.map(s=>{
    return `<li class="idea-stage" id="idea-stage-${s.id}"><div class="idea-stage-heading"><h3>${e(s.title)}</h3></div><div class="idea-body"><div class="idea-copy"><ul>${s.bullets.map(b=>`<li>${e(b)}</li>`).join('')}</ul><a class="idea-evidence" href="#role-${s.role}">${e(s.example)} <span aria-hidden="true">↗</span></a></div><a class="idea-step-image" href="assets/idea-to-sale/${s.id}-v2.png" target="_blank" rel="noopener"><img src="assets/idea-to-sale/${s.id}-v2.png" alt="Illustrative concept sketch: ${e(s.title)}" width="1536" height="1024" loading="lazy"></a></div></li>`;
  }).join('')}</ol><div class="private-label-gallery" id="private-label-products"><h3>Private-label products</h3><div class="private-label-grid">${stages.map(s=>{const more=additionalGroups[s.id];return productImage(e,s.id,s.products)+(more?productImage(e,more.id,more.brands):'')}).join('')}</div></div></div></section>`;
}
