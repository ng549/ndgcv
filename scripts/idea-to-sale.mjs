...
export function ideaToSale(e) {
  return `<section class="brand-story idea-sale-section" id="product-journey" aria-labelledby="idea-sale-heading"><div class="shell"><div class="section-heading"><span class="eyebrow">Connecting the idea with the operation</span><h2 id="idea-sale-heading">From idea to sale</h2><p>I bring the commercial decisions and the practical work together, from shaping an offer to getting it into customers’ hands. These parts overlap: I work through them with the people responsible, returning to earlier decisions as we learn more.</p></div><div class="idea-stages">${stages.map(s=>{
    return `<details class="idea-stage" id="idea-stage-${s.id}" open><summary><h3>${e(s.title)}</h3><span class="idea-disclosure" aria-hidden="true"></span></summary><div class="idea-body"><div class="idea-copy"><ul>${s.bullets.map(b=>`<li>${e(b)}</li>`).join('')}</ul><a class="idea-evidence" href="#role-${s.role}">${e(s.example)} <span aria-hidden="true">↗</span></a></div></div></details>`;
  }).join('')}</div><details class="private-label-gallery"><summary><h3>Private-label products</h3><span class="intro-toggle" aria-hidden="true"></span></summary><div class="private-label-grid">${stages.map(s=>{const more=additionalGroups[s.id];return productImage(e,s.id,s.products)+(more?productImage(e,more.id,more.brands):'')}).join('')}</div></details></div></section>`;
}
